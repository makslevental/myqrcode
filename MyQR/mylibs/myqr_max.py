...
def run(words, version=1, level='H', picture=None, colorized=False, contrast=1.0, brightness=1.0, save_name=None,
    ...
    def combine(ver, qr_name, bg_name, colorized, contrast, brightness, save_dir, save_name=None):
        ...
        if bg0.size[0] < bg0.size[1]:
            bg0 = bg0.resize((qr.size[0] - 2 * 24, (qr.size[0] - 2 * 24) * int(bg0.size[1] / bg0.size[0])))
        else:
            bg0 = bg0.resize(((qr.size[1] - 2 * 24) * int(bg0.size[0] / bg0.size[1]), qr.size[1] - 2 * 24))
                    ...
                    if not ((a == b == 0) or (a == len(aloc) - 1 and b == 0) or (a == 0 and b == len(aloc) - 1)):
                        for i in range(2 * 3 * (aloc[a] - 2), 2 * 3 * (aloc[a] + 3)):
                            for j in range(2 * 3 * (aloc[b] - 2), 2 * 3 * (aloc[b] + 3)):
                                ...
                                #         aligs.append((2*i,2*j))

        for i in range(qr.size[0] - 2 * 24):
            for j in range(qr.size[1] - 2 * 24):
                ...
                qr.putpixel((i + 2 * 12, j + 2 * 12), bg.getpixel((i, j)))
            if not (
                                                    (i < 2 * 24 and j < 2 * 24) or
                                                    (i < 2 * 24 and j > qr.size[1] - 2 * 49 + 1) or
                                                (i > qr.size[0] - 2 * 49 + 1 and j < 2 * 24) or
                                            ((i, j) in aligs) or
                                        (i % 6 == 2 and j % 6 == 2) or
                                    (i % 6 == 2 and j % 6 == 1) or
                                (i % 6 == 1 and j % 6 == 2) or
